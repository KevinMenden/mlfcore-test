...
def log_conda_environment(reports_output_dir: str):
    ...
    conda_env_filehandler = open(f'{reports_output_dir}/mlfcore_test_conda_environment.yml', "w")
    subprocess.call(['conda', 'env', 'export', '--name', 'mlfcore_test'], stdout=conda_env_filehandler)
    click.echo(click.style('Uploading conda environment report as a run artifact...', fg='blue'))
    mlflow.log_artifact(f'{reports_output_dir}/mlfcore_test_conda_environment.yml', artifact_path='reports')
